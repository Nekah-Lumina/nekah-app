...
export default function Challenges() {
  ...
  useEffect(() => {
    ...
    // Check if user needs to accept challenges
    if (LocalStorageService.isOnboardingCompleted() && !LocalStorageService.hasUserAcceptedChallenges()) {
      setShowAcceptanceModal(true);
    }
    
    // Check for reminders every 5 minutes to reduce performance impact
  ...
  const checkReminders = () => {
    try {
      // Only check reminders if user has accepted challenges
      ...
      // Show notification for due reminders (limit to prevent spam)
  ...
  const loadChallengeData = () => {
    try {
      ...
      console.log('Loading challenges for week:', pregnancyWeek);
      ...
      console.log('Optional challenges:', optional.length);
      console.log('Wellness stats:', stats);
  ...
  const getCategoryColor = (category: string) => {
    switch (category) {
      ...
      case 'mental-health': return 'bg-purple-500';
      ...
      <Card key={challenge.id} className={`relative transition-all duration-200 ${
        ...
        <CardContent className="pt-0">
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="text-xs text-gray-500 capitalize">{challenge.category.replace('-', ' ')}</span>
            <span className="text-xs text-gray-500">• {challenge.estimatedTime}</span>
            <span className="text-xs font-medium text-primary">+{challenge.points} pts</span>
          </div>
            ...
            {!isCompleted && !isSkipped && (
              <>
                {challenge.category === 'exercise' ? (
                  <Button 
                    onClick={() => setShowExerciseMap({challengeId: challenge.id, type: challenge.title.toLowerCase().includes('walk') ? 'walking' : challenge.title.toLowerCase().includes('yoga') ? 'prenatal-yoga' : challenge.title.toLowerCase().includes('swim') ? 'swimming' : 'stretching'})}
    ...
    <div className="min-h-screen bg-gradient-to-br from-pink-50 to-purple-50">
      ...
      {showAcceptanceModal && (
        <ChallengeAcceptanceModal
          ...
          onAccept={() => {
            LocalStorageService.setUserAcceptedChallenges(true);
